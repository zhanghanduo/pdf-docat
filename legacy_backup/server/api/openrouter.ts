...
export async function getOpenRouterApiKey(): Promise<string> {
  try {
    const setting = await storage.getSetting('OPENROUTER_API_KEY');
    if (setting && setting.value) {
      return setting.value;
    }
    return API_KEY;
  } catch (error) {
    console.error('Error getting OpenRouter API key from settings:', error);
    return API_KEY;
  }
}
...
(async () => {
  try {
    ...
    console.log('OpenRouter API key initialized from settings');
  } catch (error) {
    console.error('Error initializing OpenRouter API key:', error);
  }
})();
