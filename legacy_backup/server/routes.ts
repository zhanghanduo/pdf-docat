...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/settings", verifyToken, isAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/settings/:key", verifyToken, isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (setting.key.includes('API_KEY')) {
        ...
        if (value && value.length > 8) {
          const maskedValue = `${value.substring(0, 4)}${'•'.repeat(value.length - 8)}${value.substring(value.length - 4)}`;
          return res.status(200).json({ ...setting, value: maskedValue, masked: true });
        }
      }
      ...
      return res.status(500).json({ message: "Server error fetching setting" });
    }
  });

  app.post("/api/settings", verifyToken, isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      // Log setting update
      logUserAction(req.user!.id, "Updated setting", { key });
